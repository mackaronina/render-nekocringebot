...
def msg_reaction(event):
    if event.message_id in react_id:
        ...
        words = ['пиздатое', 'потужное', 'волшебное', 'ебанутое', 'некославное', 'кринжовое', 'базированное',
                 '[ДАННЫЕ УДАЛЕНЫ]', 'свеговое', 'п̞̀р̷́о̅͟к̘͞л͞͡я̻͘т̦͆о͑̕е̛͑']
...
Некославию в этот канал. Твои новости должны быть сенсационными, уникальными, креативными, провокационными'
    chat_completion = neuro.chat.completions.create(
        messages=[
            {"role": "system",
             "content": lore},
...
async def jobcheckcall():
    try:
        async with TelegramClient(StringSession(ses), api_id, api_hash) as client:
            ...
            await calls.start()
            users = await calls.get_participants(-1002484030202)
